...
const draw_chart = async (params) => {
    ...
    const canvas = document.querySelector(params.canvas);
    ...
    ctx.lineWidth = Math.min(canvas.width, canvas.height) * 0.02;
    ctx.setLineDash([]);
    ctx.lineCap = "square";
    ctx.lineJoin = "round";
    ...
    ctx.save();
    ctx.rect(margin.l, margin.t, canvas.width - margin.l - margin.r, canvas.height - margin.t - margin.b);
    ctx.clip();
